...
Balance pass, 2026-07-06 (user request, third review pass): root CLAUDE.md gained one always-loaded orientation line (primary active work = trading bot/, launch week 2026-07-06) — was over-trimmed to pure pointers.

Second review pass, 2026-07-06 (user request): (1) Worktrees section corrected — said "one active worktree", `git worktree list` shows 19 (congressional-bot + 18 under .claude/worktrees/); now documents reality with prune guidance. (2) New #flask-app anchor added to PROJECT.md, transporting the Flask sentiment-app guidance from ~/Downloads/CLAUDE.md (2026-05-28 vintage) — that file sat above the thesis repo and loaded 83 irrelevant lines into every thesis session; content condensed, all constraints kept (row_id hash, _df_cache invalidation, rate-limit back-off, security rules, smoke test). Zone-2 pointer added. Deletion of ~/Downloads/CLAUDE.md pending user approval.

Worktree prune, 2026-07-06 (user action, outside a review pass): 15 of the 18 stale `.claude/worktrees/` entries removed via `git worktree remove`; `agent-a406ac57552c79bd4` and `agent-acd81f6ef185da66b` kept (each holds uncommitted edits — `tests/test_risk_manager.py` and `bot/scraper.py` + `tests/test_scraper.py` respectively — pending review before commit or force-remove). `#worktrees` updated from 19 to 3, re-verified against `git worktree list` output. Pruned worktrees' branch commits (`worktree-agent-*`) still exist in git — not deleted, just unworktreed.
